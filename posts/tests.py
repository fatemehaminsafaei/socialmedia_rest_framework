...
class PostModelTest(TestCase):
    ...
    def setUp(self):
        self.author = User.objects.create_user(
            username='fatemeh', password='1234fa'
        )
    ...
    def test_post_create(self):
        ...
        self.assertEqual(p.text, 'unit test post')
    ...
    #     p = Post.objects.create(text='unit test post', author=self.author)
    #     self.assertEqual(p.get_comment_count(), 0)

    # def test_post_comment_count_one(self):
    #     p = Post.objects.create(text='unit test post', author=self.author)
    #     Comment.objects.create(text='unit test comment', author=self.author, post= p)
    #     self.assertEqual(p.get_comment_count(), 1)




class PostListCreatTest(APITestCase):

    @classmethod
    def setUpTestData(cls):
        user = User.objects.create_user(username='fatemeh', password='1234fa')
    ...
    def test_post_list(self):
        ...
        posts= PostSerializer(
            instance=Post.objects.filter(author=self.author),
            many=True
        )

        self.assertEqual(res.status_code, s.HTTP_200_OK)
        self.assertEqual(posts.data, res.data)
